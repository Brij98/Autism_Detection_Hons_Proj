...
class RandomForest:
    ...
    def train_random_forest(self, training_data_dir, save_mdl=False):
        ...
        self.x_train, self.x_test, self.y_train, self.y_test = process_training_data(training_data_dir)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            decision_trees = {executor.submit(trained_decision_tree, self.min_sample_split,
                                              self.max_depth, self.num_features, self.x_train, self.y_train): i
                              for i in range(self.num_tree)}

            for future in concurrent.futures.as_completed(decision_trees):
                try:
                    self.arr_trees.append(future.result())
    ...
    def __test_random_forest(self):
        ...
        accuracy_score = Utils.calculate_accuracy_score(self.y_test, y_predictions)
        print("Accuracy: ", accuracy_score)

        cf = Utils.calculate_confusion_matrix(self.y_test, y_predictions)
        print(cf[0])
        print(cf[1])
    ...
    def save_decision_trees(self, fl_name=RandomForestMdlFile):
        if self.arr_trees is not None:
        ...
        try:
            with open(fl_name, 'rb') as input_tree:
                for i in range(num_trees):
                    self.arr_trees.append(pickle.load(input_tree))
...
if __name__ == '__main__':
    ...
    ret_val = rf.predict(df.to_numpy(), True)
    print(ret_val)
